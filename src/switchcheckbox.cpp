...
void SelectionControl::checkStateSet() {
    ...
    emit stateChanged(state); // меняется тема / но не состояние кнопки
    toggle(state); // переключаьтель меняется / тема не меняется
}
...
QRect SwitchCheckBox::indicatorRect() {
...
QRect SwitchCheckBox::textRect() {
    const auto w = style.indicatorMargin.left() + style.height + style.indicatorMargin.right();
    return ltr(this) ? rect().marginsRemoved(QMargins(w, 0, 0, 0)) : rect().marginsRemoved(QMargins(0, 0, w, 0));
}

SwitchCheckBox::SwitchCheckBox(bool state, QWidget* parent) : SelectionControl(parent) {
    init();
}

SwitchCheckBox::SwitchCheckBox(bool state, const QString& text, QWidget* parent) : SwitchCheckBox(state, parent) {
    setText(text);
}

SwitchCheckBox::SwitchCheckBox(bool state, const QString& text, const QBrush& brush, QWidget* parent) : SwitchCheckBox(state, text, parent)
{
    ...
    {
        setChecked(true);
        trackBrushAnimation->setStartValue(colorFromOpacity(style.trackOnBrush, style.trackOnOpacity));
        trackBrushAnimation->setEndValue(colorFromOpacity(style.trackOnBrush, style.trackOnOpacity));
        thumbBrushAnimation->setStartValue(colorFromOpacity(style.thumbOnBrush, style.thumbOnOpacity));
...
QSize SwitchCheckBox::sizeHint() const {
    auto h = style.height;
...
void SwitchCheckBox::paintEvent(QPaintEvent *) {
...
void SwitchCheckBox::resizeEvent(QResizeEvent* e)
{
    shadowPixmap = Style::drawShadowEllipse(THUMB_RADIUS, SHADOW_ELEVATION, QColor(0, 0, 0, 70));
    SelectionControl::resizeEvent(e);
}

void SwitchCheckBox::toggle(Qt::CheckState state)
{
    if (state == Qt::CheckState::Checked || state == Qt::CheckState::PartiallyChecked)
    {
    ...
    {
        thumbPosAniamtion->interpolate((style.indicatorMargin.left() + style.indicatorMargin.right() + 2) * 2, 0);
